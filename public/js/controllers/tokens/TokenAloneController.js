SimbaApp.controller('TokenAloneController',function($rootScope, $scope, $http, $modal, $location, TokenFactory, SimbaCache, ValidateTokenInfo, AppFactory, AdsFactory, Auth ) {
    ...
    /*
     Actions
     */
    $scope.toTokenList = function (item) {
        $scope.capp =item;
        SimbaCache.setValue("app", $scope.capp);
        $scope.initTableModel($scope.capp.id);
        $location.path("/token_alone/list");
    };
});
